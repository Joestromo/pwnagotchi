...
class BluetoothSniffer(plugins.Plugin):
    __GitHub__ = ""
    __author__ = "(edited by: itsdarklikehell bauke.molenaar@gmail.com), diytechtinker, fixed by Jayofelony"
    __version__ = "0.1.4"
    __license__ = "GPL3"
    ...
    __help__ = "A plugin that sniffs Bluetooth devices and saves their MAC addresses, name and counts to a JSON file"
    __dependencies__ = {
        "apt": ["none"],
        "pip": ["none"],
    }
    ...
    def on_ui_update(self, ui):
        ...
        if current_time - self.last_scan_time >= self.options.get("timer", 5):
        ...
        try:
            ...
            for line in inq_output.splitlines()[1:]:
                ...
                if mac_address in self.data and len(self.data) > 0:
                    ...
                    last_seen_time = int(
                        datetime.strptime(
                            self.data[mac_address]["last_seen"], "%H:%M:%S %d-%m-%Y"
                        ).timestamp()
                    )
                    import logging
import os
        ...
        try:
            ...
            for line in inq_output.splitlines()[1:]:
                ...
                if mac_address in self.data and len(self.data) > 0:
                    ...
                    interval = self.options.get("count_interval", 60)  # default 60 seconds
if current_time - last_seen_time >= interval:
    ...
                        self.data[mac_address]["count"] += 1
                        self.data[mac_address]["last_seen"] = time.strftime(
    ...
    def get_device_manufacturer(self, mac_address):
        ...
        cmd_info = (
            f"hcitool info {mac_address} | grep 'Manufacturer:' | cut -d ' ' -f 2-"
        )
        try:
            logging.info(
    ...
    def bt_sniff_info(self):
        ...
        if num_devices > 0:
            num_unknown = sum(
                ...
                if device["name"] == "Unknown" or device["manufacturer"] == "Unknown"
            )
