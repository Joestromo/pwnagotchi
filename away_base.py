...
class AwayBase(plugins.Plugin):
    __GitHub__ = ""
    __author__ = "(edited by: itsdarklikehell bauke.molenaar@gmail.com), @nope"
    __version__ = "1.0.0"
    __license__ = "GPL3"
    __description__ = (
    ...
    def on_wifi_update(self, agent, access_points):
        ...
        if self.ready == 1 and "Not-Associated" in result:
            potfile = _run(
                "cat /root/handshakes/wpa-sec.cracked.potfile | awk -F: '{print $3 \":\" $4}'"
            ).splitlines()
            pwned_networks = {}
            for line in potfile:
                network = line.split(":")
            ...
            for network in access_points:
                if (
                    network["hostname"] in pwned_networks
                    and network["hostname"] not in self.used_networks
                ):
                    signal_strength = network["rssi"]
                    channel = network["channel"]
                    _log(
                        "FOUND cracked network nearby on channel %d (rssi: %d)"
                    ...
                    if signal_strength >= self.options["minimum_signal_strength"]:
                        ...
                        _connect_to_target_network(
                            self, agent, network["hostname"], channel, password
                        )
                    else:
                        _log(
    ...
    def on_epoch(self, agent, epoch, epoch_data):
        ...
        current_network = l[
            matches = [m.start() for m in re.finditer(r'"', l)]
if len(matches) < 2:
    continue  # or return, depending on context

value = l[matches[0] + 1 : matches[1]]
        ]
        if (
            current_network in self.used_networks
            and self.used_networks[current_network]
